...
Since 2017 was in the past, we can actually see how each of these two models did in terms of prediction. In the same folder as this lab, there is an Excel spreadsheet (also saved as a CSV file) called **HawaiiCO2monthlyData** that I downloaded in 2021. It includes a column for CO2 values, that I have highlighted, whose values closely match those in **CO2Hawaii** (they are only different due to round off error and later data correction). 
